# This Makefile is for the Vigil::Crypt extension to perl.
#
...
#     ABSTRACT => q[Encryption and Hashing wrapper for ChaCha20-Poly1305 and Argon2]
...
#     NAME => q[Vigil::Crypt]
#     PREREQ_PM => { Bytes::Random::Secure=>q[0], Crypt::Argon2=>q[0], Crypt::AuthEnc::ChaCha20Poly1305=>q[0], Digest::SHA=>q[0], Test::More=>q[1.001014], Time::Local=>q[0] }
#     TEST_REQUIRES => { Test::More=>q[1.001014] }
#     VERSION_FROM => q[lib/Vigil/Crypt.pm]
...
DISTNAME = Vigil-Crypt
DISTVNAME = Vigil-Crypt-2.1.2
...
Crypt.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Vigil::Crypt\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'Encryption and Hashing wrapper for ChaCha20-Poly1305 and Argon2'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Vigil-Crypt" >> META_new.yml
	$(NOECHO) $(ECHO) no_index: >> META_new.yml
	$(NOECHO) $(ECHO) "  directory:" >> META_new.yml
	$(NOECHO) $(ECHO) "    - t" >> META_new.yml
	$(NOECHO) $(ECHO) "    - inc" >> META_new.yml
	$(NOECHO) $(ECHO) requires: >> META_new.yml
	$(NOECHO) $(ECHO) "  Bytes::Random::Secure: '0'" >> META_new.yml
	$(NOECHO) $(ECHO) "  Crypt::Argon2: '0'" >> META_new.yml
	$(NOECHO) $(ECHO) "  Crypt::AuthEnc::ChaCha20Poly1305: '0'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Encryption and Hashing wrapper for ChaCha20-Poly1305 and Argon2\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Vigil-Crypt\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "         \"requires\" : {" >> META_new.json
	$(NOECHO) $(ECHO) "            \"Bytes::Random::Secure\" : \"0\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Crypt::Argon2\" : \"0\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Crypt::AuthEnc::ChaCha20Poly1305\" : \"0\"," >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) "<SOFTPKG NAME=\"Vigil-Crypt\" VERSION=\"2.1.2\">" > Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "    <ABSTRACT>Encryption and Hashing wrapper for ChaCha20-Poly1305 and Argon2</ABSTRACT>" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Jim Melanson &lt;jmelanson1965@gmail.com&gt;</AUTHOR>" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,010000,0,0\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Bytes::Random::Secure\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Crypt::Argon2\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Crypt::AuthEnc::ChaCha20Poly1305\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Digest::SHA\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Time::Local\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <ARCHITECTURE NAME=\"MSWin32-x64-multi-thread-5.32\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "        <CODEBASE HREF=\"\" />" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) "    </IMPLEMENTATION>" >> Vigil-Crypt.ppd
	$(NOECHO) $(ECHO) ^</SOFTPKG^> >> Vigil-Crypt.ppd
...
pm_to_blib({@ARGV}, '$(INST_LIB)\auto', $$filter, '$(PERM_DIR)')" -- $(PM_FILTER) -- \
	  lib/Vigil/Crypt.pm blib\lib\Vigil\Crypt.pm 
